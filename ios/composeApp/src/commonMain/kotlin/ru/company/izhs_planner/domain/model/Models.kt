...
enum class RoofType {
    FLAT, GABLED, HIPPED, MANSARD
}

enum class FoundationType {
    STRIP, TILE, COLUMN, SCREW_PILE
}
...
data class Project(
    ...
    val livingArea: Float get() = floors.sumOf { floor -> floor.rooms.filter { it.type != RoomType.CLOSET && it.type != RoomType.BOILER_ROOM && it.type != RoomType.UTILITY }.sumOf { it.area.toDouble() } }.toFloat()
}
