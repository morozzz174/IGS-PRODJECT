...
class AIManager(private val llm: LLMInference) {
    private var isModelLoaded = false
    private val plannerAgent = PlannerAgent(llm)
    private val calculatorAgent = CalculatorAgent(llm)
    private val codesCheckerAgent = CodesCheckerAgent(llm)

    fun initialize(modelPath: String): Boolean {
        isModelLoaded = llm.init(modelPath)
        return isModelLoaded
    }

    fun isReady(): Boolean = isModelLoaded && llm.isInitialized()
    ...
    fun generateResponse(
        ...
        try {
            ...
            val fullPrompt = buildPrompt(systemPrompt, userMessage, chatHistory, projectInfo)

            if (isReady()) {
                val response = when (agentType) {
                    AgentType.PLANNER -> plannerAgent.generate(fullPrompt)
                    AgentType.CALCULATOR -> calculatorAgent.generate(fullPrompt)
                    AgentType.CODES_CHECKER -> codesCheckerAgent.generate(fullPrompt)
                }
                AIResponse(content = response, isComplete = true)
            } else {
                AIResponse(
                    content = "ИИ-модель не загружена. Пожалуйста, загрузите модель в настройках.",
                    isComplete = false
                )
            }
        } catch (e: Exception) {
    ...
    companion object {
        const val MODEL_FILENAME = "qwen2.5-3b-instruct-q4_0.bin"
...
private fun getSystemPrompt(agentType: AgentType): String = when (agentType) {
    AgentType.PLANNER -> """
        ...
        Твоя задача — помогать с планировкой помещений, зонированием, инсоляцией.
        Отвечай строго в рамках СП 11-101-95, СП 55.13330, СП 42.13330.
        Основные принципы:
        ...
        - Санузлы не должны примыкать к кухне и жилым комнатам
