...
class AIManager(private val context: Context) {
    ...
    private fun getModelFile(): File {
        ...
        return File(modelDir, "qwen2.5-3b-instruct-q4_0.bin")
    }
    ...
    suspend fun initialize(): Boolean = withContext(Dispatchers.IO) {
        try {
            ...
            if (modelFile.exists() && modelFile.length() > 1_000_000_000L) {
                ...
                val loaded = LLMInference.init(modelFile.absolutePath)
                isModelLoaded = loaded
                _downloadState.value = if (loaded) DownloadState.COMPLETED else DownloadState.FAILED
                _downloadProgress.value = if (loaded) 1f else 0f
            } else {
    ...
    fun isReady(): Boolean = isModelLoaded && LLMInference.isInitialized()
