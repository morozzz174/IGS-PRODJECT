...
class CloudRepositoryImpl : CloudRepository {
    ...
    override suspend fun fetchProjects(userId: String): List<Project> {
        ...
        return try {
            ...
            val projects = snapshot.documents.mapNotNull { doc ->
                try {
                    doc.toObject(CloudProject::class.java)?.toProject()
                } catch (e: Exception) {
                    null
                }
            }
    ...
    override suspend fun syncAllProjects(userId: String): Boolean {
        ...
        return try {
            ...
            for (doc in snapshot.documents) {
                try {
                    val cloudProject = doc.toObject(CloudProject::class.java)
    ...
    data class CloudProject(
        ...
        fun toProject(): Project {
            val floorList = try {
                val type = object : com.google.gson.reflect.TypeToken<List<ru.company.izhs_planner.domain.model.Floor>>() {}.type
                com.google.gson.Gson().fromJson<List<ru.company.izhs_planner.domain.model.Floor>>(floorsJson, type)
            } catch (e: Exception) {
                emptyList()
            }
